...
class SwarmTaskBroker:
    ...
    def flush_epoch_docket(self, base_dir: str = "src/engine/ledgers") -> Dict[str, Any]:
        with self._lock:
            ...
            # 2. Write individual epoch snapshot
            ...
            # 4. Write individual task receipts
            for r in rows:
                rcpt_path = os.path.join(receipts_dir, f"{r['task_id']}.json")
                with open(rcpt_path, "w") as f:
                    json.dump(r, f, indent=2)

            # 5. Compaction: prune epoch snapshots older than latest 48 runs to prevent git bloat
...
def main():
    ...
    # 2. Ingest rotating sector workload batch
    ...
    # Flush epoch and persist to disk
