# Clerk slots — stagger wheel

Stamp: 2026-09-23T14:32Z
Law: one writer per minute. FOREST already owns harvest minutes. Do not land on them.

## Pick: 5 clerks × ~5 min, twice an hour

Not 15 × 1 min. Fifteen warm workflows would sit on every harvest slot we already run and burn Actions for empty desks.

| minute | clerk | job | writes main? |
|---:|---|---|---|
| :01 / :31 | `unity:forensics.clerk` | name the bot, stamp lesson | no |
| :06 / :36 | `unity:audit.clerk` | street HEAD of the hall | no |
| :13 / :43 | `unity:design.clerk` | read tickets, write blueprint if UNKNOWN | AGENT/BLUEPRINTS only |
| :19 / :49 | `unity:develop.clerk` | pick READY plates | root files only if READY |
| :23 / :53 | `unity:wattage.clerk` | score idle swarms | no |

Publisher.clerk is **not** on this wheel. Critical tickets tick on-drop. See `AGENT/TICK.md` and `AGENT/ESCALATE.md`.

## Occupied minutes (do not take)

FOREST live: :02 :04 :07 :09 :12 :17 :22 :27 and +30.
Old factory-audit `*/5` was the collision hog. It now sits on :06 / :36 only.
Old forensics `:17` sat on OMHA + plate-verify. It now sits on :01 / :31 only.

## 15 × 1 min

Illegal until fifteen KINDS have a queue. Then they take leftover minutes (:03 :08 :11 :14 :16 :18 :21 :24 :26 :28 :29 …) one kind at a time. Empty yml is theater. `factory.wattage` holes it.
