...
(function () {
  ...
  window.DC_MEDICAL.prompt = function () {
    var email = window.prompt("Institutional email (.org / .gov / .gc.ca):", "");
    if (email == null) return;
    var mark = window.prompt("Optional SEAL-1 mark (or leave blank):", "") || "";
    var org = window.prompt("Organization name:", "") || "";
    var d = window.DC_MEDICAL.open(email, org, mark);
    var n = document.getElementById("sealDenyNote");
    if (!d || !d.ok) {
      if (n) {
        n.style.display = "block";
        n.textContent = "Access denied. SEAL is for authorized .org / .gov / Phase-3 agencies (or Dualis SEAL-1). Public checkout is not offered.";
      } else {
        alert("Access denied — institutional SEAL only.");
      }
      return d;
    }
    if (n) {
      n.style.display = "block";
      n.style.color = "var(--accent-mint)";
      n.style.borderColor = "rgba(52,211,153,0.35)";
      n.textContent = "Institutional access granted (" + (d.kind || "ok") + "). SEAL depth path unlocked for this session.";
    }
    return d;
  };
