...
def test_attack_1_stampede_race_condition():
    ...
    def racer(worker_id):
        barrier.wait()  # Synchronized release
...
def test_attack_2_worker_sudden_death_and_lease_recovery():
    ...
    # Worker A claims task with a 1-second lease and crashes (does not complete)
    ...
    # Worker B immediately tries to claim it while lease is active
    ...
    print(" • Sleeping 1.1s to allow lease to expire...")
    time.sleep(1.1)

    # Worker B tries to claim expired task
...
def test_attack_3_hostile_sql_injection_and_poison_payload():
    ...
    # Attempt SQL injection via task_id
    ...
    # Attempt math poison (NaNs, infinities)
    print(" • Ingesting non-finite float (inf/NaN)...")
    res_nan = kernel.evaluate_state_manifold(q=float('nan'), p=1.0)
    # math.isnan check
    assert math.isnan(res_nan["q_next"]) or not res_nan.get("conserved", True), "Kernel failed to catch NaN"
...
def test_attack_4_invariant_circuit_breaker_trip():
    ...
    # Boundary timer injected with artificial 5ms latency
