(function () {
  ...
  $("makeKey").onclick = async function () {
    if (!window.PublicKeyCredential) { $("keyOut").textContent = "No WebAuthn on this browser."; return; }
    try {
      const cred = await navigator.credentials.create({
        publicKey: {
          ...
          user: {
            ...
            name: $("email").value || "unity",
            displayName: $("name").value || "unity",
          },
      ...
      $("keyOut").textContent = (e && e.name) || "cancelled";
    }
  };
  ...
  $("signEth").onclick = async function () {
    ...
    try {
      ...
      $("ethOut").textContent = "Wallet refused. Address still sealable.";
    }
  };
  ...
  function packet() {
    ...
    const orgOk = !org || email.endsWith("@" + org);
    return {
      schema: "unity.id.v1",
      ...
      look: {
        ...
        org_domain: org || null,
        org_matches_email: org ? orgOk : null,
      },
      ...
      declaration: { checked: !!($("attest") && $("attest").checked), text: "chairs mine; Dualis is meter not licence issuer" },
  ...
  $("seal").onclick = function () {
    ...
    if (!validEmail($("email").value)) { $("log").textContent = "Need a real email you can open."; return; }
    if (!($("muni").value || "").trim()) { $("log").textContent = "Need a municipality claim."; return; }
    const p = packet();
  ...
  $("mail").onclick = function () {
    ...
    const subject = encodeURIComponent("Unity ID bind " + p.at.slice(0, 10));
    const body = encodeURIComponent("Host: " + p.host + "\nEmail: " + p.look.email + "\nMuni: " + p.look.municipality + "\nTime: " + p.at + "\n\nKeep this mail. Dualis did not send it from a server.");
    location.href = "mailto:" + p.look.email + "?subject=" + subject + "&body=" + body;
  };
