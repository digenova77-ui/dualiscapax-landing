/* DualisCapax pipe — storyboard surface engine.
 * Baseline: DualisCapax_Website_Storyboard_Refinement_2026-09-02
 * Circuit-tunnel fly-through prohibited. Lifestyle grains render as sector cards only.
 */
(function () {
  ...
  var SECTORS = [
    ...
    {id:"sec-05",code:"SEC-05",name:"Hospitality & Food Service Logistics",short:"Tomasso's & Jim's",img:"https://images.unsplash.com/photo-1556910103-1c02745aae4d?auto=format&fit=crop&w=1600&q=80",alt:"High-volume commercial kitchen prep line",thesis:"Tomasso's & Jim's 4-step onboarding recovers CAD $214,800 / yr of operational drag at $0 upfront. Year-1 net retention floor 81%. Recipes unaltered.",metrics:[["Recoverable","CAD $214.8k"],["Net retention","81.0%"],["Upfront","$0.00"],["Access","CLOSED"]],steps:[["Days 1-15","POS, meter, invoice, and schedule ingress. CPA seals the baseline."],["Days 16-45","Vectors 1 and 2: predictive labor staging and mozzarella batch controls."],["Days 46-75","Vectors 3-5: deck-oven peak shave, delivery staging, asset upkeep."],["Days 76-90","Independent CPA audit and capital distribution. Culinary sovereignty held."]]},
  ...
  var active = SECTORS[4];
  var thoughts = [];
  function $(id) { return document.getElementById(id); }
  function log(obj) {
    thoughts.unshift(obj);
  ...
  function paintHero(sec) {
    ...
    if (cap) cap.textContent = sec.code + " \u00b7 " + sec.name;
    ...
    document.querySelectorAll(".card").forEach(function (el) {
      el.classList.toggle("on", el.getAttribute("data-id") === sec.id);
    });
  ...
  if (raw) select(raw, "hash"); else select(active.id, "boot");
  var beats = [
  ...
  setInterval(function () {
    log({ t: Date.now(), beat: beats[bi % beats.length], sector: active.code });
    bi++;
  }, 4200);
})();
